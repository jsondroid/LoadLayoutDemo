...
//        float yoffset =  mYAxis.getYOffset();
        ...
        /**新加的*/
        if(mYAxis.getLeftYtopDes()!=null){
            float width = Utils.calcTextWidth(mAxisLabelPaint,mYAxis.getLeftYtopDes() );
            c.drawText(mYAxis.getLeftYtopDes(),  mViewPortHandler.offsetLeft() + xoffset+width,mViewPortHandler.offsetTop() , mAxisLabelPaint);
        }
    }
    ...
    public void renderAxisLine(Canvas c) {
        ...
        if (mYAxis.getAxisDependency() == AxisDependency.LEFT) {
            c.drawLine(mViewPortHandler.contentLeft(), mViewPortHandler.contentTop(), mViewPortHandler.contentLeft(),
                    mViewPortHandler.contentBottom(), mAxisLinePaint); //原来的

//            c.drawLine(mViewPortHandler.contentLeft(), mViewPortHandler.contentTop()-mViewPortHandler.offsetTop()/2, mViewPortHandler.contentLeft(),
//                    mViewPortHandler.contentBottom(), mAxisLinePaint);//修改的
        } else {
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        for (int i = from; i < to; i++) {
            ...
            if(mYAxis.getLabelRotationAngle()==0){
                c.drawText(text, fixedPosition, positions[i * 2 + 1] + offset, mAxisLabelPaint);//原始的写法
            }else{
                /**修改过的写法*/
//                float width = Utils.calcTextWidth(mAxisLabelPaint, text);
                float height = Utils.calcTextHeight(mAxisLabelPaint, text);
                Utils.drawXAxisValue(c, text, mViewPortHandler.contentLeft()-10, positions[i * 2 + 1] + offset-height/2, mAxisLabelPaint, pointF, mYAxis.getLabelRotationAngle());
            }
        }
    }
